...
export function MainNav() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full main-nav">
      ...
      <div className="bg-sakura text-white py-1 px-4 text-center text-sm font-medium dark:text-sakura-foreground">
        <AnimatedText
          text="2025シーズン 新メンバー募集中!"
          animation="wave"
          speed={0.5}
        />
      </div>
      ...
      <div className="container flex h-16 items-center justify-between bg-background">
        ...
        <div className="flex gap-2 md:gap-4 items-center">
          <Link href="/" className="flex items-center space-x-2">
            ...
            <span className="font-bold text-xl md:text-2xl children-font text-sakura-dark dark:text-sakura-light">
              <span className="text-sakura animate-pulse dark:text-sakura-light">
                サクラ
              </span>
              野球クラブ
            </span>
          ...
          <div className="flex items-center gap-6">
            {routes.map((route, index) => (
              <Link
                ...
                href={route.href}
                className={cn(
                  "nav-link text-sm font-medium transition-colors hover:text-sakura title-animated dark:text-foreground dark:hover:text-sakura-light"
                )}
              >
                <AnimatedText
                  text={route.label}
        ...
        <div className="flex items-center gap-2 md:hidden">
          ...
          <Button
            ...
            size="icon"
            onClick={toggleMenu}
            className="text-sakura dark:text-sakura-light"
          >
            {isMenuOpen ? (
              <X className="h-5 w-5" />
            ) : (
            ...
            <nav className="flex flex-col gap-6">
              {routes.map((route, index) => (
                <Link
                  ...
                  href={route.href}
                  className={cn(
                    "text-lg font-medium transition-colors hover:text-sakura children-font flex items-center dark:text-foreground dark:hover:text-sakura-light"
                  )}
                  onClick={toggleMenu}
                >
                  <Cherry className="h-4 w-4 mr-2 text-sakura animate-pulse dark:text-sakura-light" />
                  <AnimatedText
                    text={route.label}
                    animation="slideUp"
                    delay={index * 0.1}
